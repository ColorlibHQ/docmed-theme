...
function docmed_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'docmed' ),
        'next_string'     => esc_html__( '&raquo;', 'docmed' ),
        'before_output'   => '<div class="docmed-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'docmed_pagination_defaults', $defaults )
    );
