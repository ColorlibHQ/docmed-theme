...
function docmed_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'docmed' ) . '</span></li>';
...
// Blog categories
function docmed_featured_post_cat(){
...
// social media
if ( ! function_exists( 'docmed_social' ) ) {
	function docmed_social( $args = array() ) {
		...
		$url = docmed_opt( 'docmed_social_url' );
...
// header cart count
function docmed_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( DOCMED_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'docmed' ); ?>">
...
function docmed_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'docmed_contact7_form_content', 10, 2 );